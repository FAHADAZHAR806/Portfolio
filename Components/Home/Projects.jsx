...
{Projects.map((item,index)=>(
  <div key={index} data-aos="fade-up" className="grid lg:grid-cols-2 grid-cols-1 max-w-5xl m-auto py-10 px-4 text-white gap-4">

<div className='grid gap-4  sm:grid-col-1 sm:gap-6  text-[#FFFFFF] sm:p-10'>
        <p className='text-4xl'>{item.PName}</p>
        <p  >{item.Para}</p>
        <button className='rounded-2xl w-40 text-left  h-10 text-[#F50D32] sm:w-40 sm:h-10' onClick={() => window.open('https://nimble-paprenjak-921093.netlify.app/')}>{item.buttontext}</button>
